...
namespace app\models\shop\document;
...
/**
 * This is the model class for table "shop_document".
 *
 * @property int $id
 ...
 * @property Shop $shop
 */
class ShopDocument extends \yii\db\ActiveRecord
{
    public $files = [];
    /**
     * {@inheritdoc}
    ...
    public function getPath() {
        if ($this->file) {
            $path = File::FILE_DOCUMENT.$this->file->object_id.'/file/'.$this->file->url;
            if (is_file($path)) {
                return '/'.$path;
            }
        }
    ...
    public function getFile() {
        return $this->hasOne(File::className(), ['object_id'=>'id'])->andOnCondition(['type'=>'document', 'main'=>1]);
    }
}
