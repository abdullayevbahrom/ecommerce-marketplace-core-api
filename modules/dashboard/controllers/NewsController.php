...
class NewsController extends Controller {
    ...
    public function actionIndex() {
        ...
        $query = News::find()->with('image')->where(['shop_id'=>$shop->id]);
    ...
    public function actionDetail($id) {
        ...
        $model = News::find()->with('image')->where(['id'=>$id, 'shop_id'=>$shop->id])->one();
    ...
    public function actionCreate($id = null) {
        ...
        if ($id) {
            $model = News::findOne(['id'=>$id, 'shop_id'=>$shop->id]);
            if (!$model) {
                Yii::$app->response->statusCode = 404;
                return ['errors'=>['id'=>'Новость не найдена']];
            }
        }
        ...
        if ($model->saveObject(true)) {
            ...
            if ($image->imageFiles[] = UploadedFile::getInstanceByName('photo')) {
                $image->uploadPhoto($model->id, 'news');
            }
        }

        $model = News::find()->with('image')->where(['id'=>$model->id, 'shop_id'=>$shop->id])->one();
        return ['data'=>$model];
    }
    ...
    public function actionRemove() {
        ...
        $model = News::find()->with('image')->where(['id'=>$post['id'], 'shop_id'=>$shop->id])->one();
