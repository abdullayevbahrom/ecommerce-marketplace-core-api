...
class SliderController extends Controller{
    ...
    public function actionCreate($id = null) {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->saveObject()) {
                Yii::$app->session->setFlash('slider_saved', 'Слайдер успешно сохранен');
                return $this->redirect(['/shop/slider/view', 'id'=>$model->id]);
            }
        }
    ...
    public function actionRemove($id) {
        ...
        if ($this->user && ($this->user->role != User::ROLE_USER) && $model && $model->removeObject()) {
            Yii::$app->session->setFlash('slider_removed', 'Слайдер успешно удален');
        }

        return $this->redirect(['/shop/slider']);
    }
    ...
    public function actionLock($id) {
        ...
        if ($model->status == 1) {
            $model->status = 2;
            $msg = 'Слайдер успешно заблокирован';
        } else {
            $model->status = 1;
            $msg = 'Слайдер успешно разблокирован';
        }
