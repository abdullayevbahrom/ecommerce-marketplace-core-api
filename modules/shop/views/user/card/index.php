                ...
                <div class="box box-info color-palette-box">
                    <div class="box-header with-border">
                        <div class="box-title pull-right" style="font-size: 14px">
                            <a href="<?=Yii::$app->urlManager->createUrl(['/shop/user/card-create', 'id'=>Yii::$app->request->get('id')])?>" class="btn btn-success">
                    ...
                    <div class="box-body" id="item-block">
                        <?= GridView::widget([
                            ...
                            'summary' => "Страница {begin} - {end} из {totalCount} карт<br/><br/>",
                            'emptyText' => 'Карт нет',
                            'rowOptions' => function ($model, $index, $widget, $grid) {
                                return [
                                    'id' => $model['id'],
                                    'user_id' => $model['user_id'],
                                    'url' => Yii::$app->urlManager->createUrl('/shop/user/card-view').'?id='.$model['user_id'].'&card_id='.$model['id']
                                ];
                            },
                            'tableOptions' => [
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'encodeLabel' => false,
                                    'filter' => Html::activeDropDownList($searchModel, 'card_type_id', $card_types, ['class'=>'form-control select2','prompt' => 'Выбрать']),
                                    'value' => function ($model, $key, $index, $column) {
                                        return $model->cardType ? $model->cardType->name_ru : 'Не указано';
                                    },
                                ],
                                [
                                    'attribute'=>'card_number',
                                    'label'=>'<i class="fa fa-sort"></i> Номер карты',
                                    'encodeLabel' => false,
                                ],
                                [
                                    'attribute'=>'card_expire',
                                    'label'=>'<i class="fa fa-sort"></i> Срок',
                                    'encodeLabel' => false,
                                ],
                                [
                                ...
                                [
                                    ...
                                    'buttons' => [
                                        'update' => function ($url, $model) {
                                            return Html::a('<span class="glyphicon glyphicon-pencil"></span>', Yii::$app->urlManager->createUrl(['/shop/user/card-create', 'id'=>$model->user_id, 'card_id'=>$model->id]), ['class'=>'btn btn-warning']);
                                        },
                                        'delete' => function ($url, $model) {
                                            return Html::a('<span class="glyphicon glyphicon-trash"></span>', Yii::$app->urlManager->createUrl(['/shop/user/card-remove', 'id'=>$model->user_id, 'card_id'=>$model->id]), ['class'=>'btn btn-danger remove-object']);
                                        }
                                    ],
                                ]
                            ],
