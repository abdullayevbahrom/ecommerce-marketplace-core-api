...
class ModeratorController extends Controller{
    ...
    public function actionCreate() {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            ...
            if ($model->saveObject(User::ROLE_MODERATOR, 1)) {
                $moderator_access = Yii::$app->request->post('User')['moderator_access'] ?? [];
                
                ModeratorAccess::deleteAll(['user_id' => $model->id]);
                
                if (!empty($moderator_access)) {
                    foreach ($moderator_access as $access_id) {
                        $access = new ModeratorAccess();
                        $access->user_id = $model->id;
                        $access->moderator_id = $access_id;
                        $access->save();
                    }
                }
            ...
            return $this->redirect(['/admin/moderator/view', 'id'=>$model->id]);
        }
    ...
    public function actionRemove($id) {
        ...
        return $this->redirect(['/admin/moderator']);
    }
}