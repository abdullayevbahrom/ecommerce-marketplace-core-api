...
class ShopOfertaController extends Controller
{
    ...
    public function actionCreate($id = null) {
        ...
        if ($id) {
            $model = ShopOferta::find()->with('file')->where(['id'=>$id])->one();
            if (!$model) {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->saveObject()) {
                Yii::$app->session->setFlash('shop_oferta_saved', 'Saved');
                return $this->redirect(['/admin/shop-oferta/view', 'id'=>$model->id]);
            }
        }
    ...
    public function actionRemove($id) {
        ...
        if ($this->user && ($this->user->role != User::ROLE_USER) && $model && $model->removeObject()) {
            Yii::$app->session->setFlash('shop_oferta_removed', 'Deleted');
        }

        return $this->redirect(['/admin/shop-oferta']);
    }

    public function actionLock($id) {
        $model = ShopOferta::findOne($id);
