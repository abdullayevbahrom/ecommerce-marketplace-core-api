        ...
        <div class="box box-info color-palette-box">
            <div class="box-header with-border">
                <?php if (($model->delivery && $model->delivery->id != 3) || !$model->delivery) { ?>
                    <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 3]); ?>"
                        class="btn btn-info"><i class="fa fa-check"></i> На доставке / ожидается оплата</a>
                    <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 4]); ?>"
                        class="btn btn-info"><i class="fa fa-check"></i> В пути / Отправлен</a>
                <?php } ?>
                <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 5]); ?>"
                    class="btn btn-info"><i class="fa fa-check"></i> Доставлен</a>

                <?php if (($model->delivery && $model->delivery->id != 3) || !$model->delivery) { ?>
                    <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 10]); ?>"
                        class="btn btn-danger"><i class="fa fa-check"></i> Возврат</a>
                <?php } ?>
                <?php if ($model->status == 0) { ?>
                    <div class="pull-right">
                        <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 1]); ?>"
                            class="btn btn-success"><i class="fa fa-check"></i> Принять</a>
                        <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/accept', 'id' => $model->id, 'status' => 2]); ?>"
                            class="btn btn-danger"><i class="fa fa-remove"></i> Отклонить</a>
                    </div>
                <?php } ?>
            </div>
            <div class="box-body">
                <table class="table table-striped">
                            ...
                            <?php if ($model->hasBtsIntegration()): ?>
                                <div class="bts-controls" style="margin-bottom: 15px;">
                                    <button type="button" class="btn btn-info btn-sm"
                                        onclick="updateBtsStatus(<?= $model->id ?>)">
                                        <i class="fa fa-refresh"></i> Обновить статусы BTS
                                    </button>
                                    <button type="button" class="btn btn-success btn-sm"
                                        onclick="getBtsTracking(<?= $model->id ?>)">
                                ...
                                <div id="bts-status-list">
                                    <?php foreach ($model->orderProducts as $orderProduct): ?>
                                        <?php if ($orderProduct->bts_id): ?>
                                            <div class="bts-item"
                                                style="margin-bottom: 10px; padding: 10px; border: 1px solid #ddd; border-radius: 3px;"
                                                data-bts-id="<?= $orderProduct->bts_id ?>">
                                                <div style="display: flex; justify-content: space-between; align-items: center;">
                                                    ...
                                                    <div>
                                                        <?php
                                                        $statusLabel = Yii::$app->bts->getBtsStatusLabel($orderProduct->bts_status, 'ru');
                                                        $statusClass = Yii::$app->bts->getStatusColorClass($orderProduct->bts_status);
                                                        ?>
                                                ...
                                                <?php if ($orderProduct->bts_price): ?>
                                                    <div style="margin-top: 5px;">
                                                        <small><strong>Стоимость доставки:</strong>
                                                            <?= number_format($orderProduct->bts_price, 2) ?> сум</small>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                        <?php endif; ?>
                                ...
                                <?php if ($model->delivery_cost): ?>
                                    <div style="margin-top: 10px; font-weight: bold;">
                                        <strong>Общая стоимость доставки:</strong>
                                        <?= number_format($model->delivery_cost, 2) ?> сум
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td><?= $model->user && $model->user->name ? '<a href="' . Yii::$app->urlManager->createUrl(['/admin/user/view', 'id' => $model->user->id]) . '" target="_blank">' . $model->user->name . '</a>' : '-'; ?>
                        </td>
                    </tr>
                    <tr>
                        <td>Адрес:</td>
                    ...
                    <?php foreach ($products as $product) { ?>
                        <?php if ($product->product) { ?>
                            <div class="attachment-block clearfix">
                                <a href="<?= Yii::$app->urlManager->createUrl(['/admin/product/view', 'id' => $product->product->id]); ?>"
                                    target="_blank"><img class="attachment-img" src="<?= $product->product->getPhoto(); ?>"
                                        alt="Attachment Image"></a>
                                <div class="attachment-pushed">
                                    ...
                                    <div class="attachment-text">
                                        <table class="table table-striped">
                                            <tr>
                                                <td>Название:</td>
                                                <td><a href="<?= Yii::$app->urlManager->createUrl(['/admin/product/view', 'id' => $product->product->id]); ?>"
                                                        target="_blank"><?= $product->product->name_ru; ?></a></td>
                                            </tr>
                                            <tr>
                                                <td>Статус:</td>
                                                <td>
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $product->delivery ? $product->delivery->name_ru : '<small class="label bg-red">Не указан</small>'; ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>Сума за 1 шт.:</td>
                                                <td><?= number_format($product->product_price / $product->amount, 0, '.', ' ') ?> UZS
                                                </td>
                                            </tr>
                                            <tr>
                                                <td>Количество:</td>
                                            ...
                                            <tr>
                                                <td>Итоговая сума:</td>
                                                <td><?= '(' . number_format($product->product_price / $product->amount, 0, '.', ' ') . ' x ' . $product->amount . ') + ' . number_format($product->delivery_cost, 0, '.', ' ') . ' = ' . number_format($product->price, 0, '.', ' ') . ' UZS'; ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($product->hasReviews()) { ?>
                                                        ...
                                                        <?php foreach ($product->productReviews as $index => $review) { ?>
                                                            <div
                                                                style="border-left: 4px solid #ddd; padding-left: 15px; margin: 10px 0; background-color: #fafafa; padding: 10px;">
                                                                ...
                                                                <?php if ($review->status_comment) { ?>
                                                                    <div style="margin-top: 5px;">
                                                                        <div class="alert alert-info" style="margin-bottom: 0; padding: 8px;">
                                                                            <strong><i class="fa fa-info-circle"></i> Комментарий
                                                                                модератора:</strong><br>
                ...
                <div class="box-tools pull-right">
                    ...
                    <div class="btn-group" style="margin-right: 10px;">
                        <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/create-didox-invoice', 'id' => $model->id]); ?>"
                            class="btn btn-info btn-sm" onclick="return confirm('Создать счет-фактуру автоматически?')">
                                ...
                                <?php foreach ($didoxDocuments as $doc): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php endif; ?>
                                            <br><small class="text-muted">DIDOX:
                                                <?= Html::encode($doc->didox_doc_type ?: '002') ?></small>
                                        </td>
                                        <td>
                                            <?php
                                            if ($doc->didox_id) {
                                                ...
                                                switch ($doc->didox_status) {
                                                    ...
                                                    case 1:
                                                        ...
                                                        break; // Waiting partner
                                                    case 2:
                                                        $statusColor = 'label-primary';
                                                        break; // Waiting your signature
                                                    case 3:
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="<?= Yii::$app->urlManager->createUrl(['/admin/didox/view', 'id' => $doc->id]) ?>"
                                                    class="btn btn-primary btn-xs" title="Просмотреть документ">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                                <?php if (!$doc->didox_id): ?>
                                                ...
                                                <?php if ($doc->canBeSignedInDidox()): ?>
                                                    <button type="button" class="btn btn-success btn-xs" title="Подписать"
                        ...
                        <div class="col-md-12">
                            <div class="alert alert-info">
                                ...
                                <ul style="margin-bottom: 0;">
                                    ...
                                    <li>Подключенных к DIDOX:
                                        <strong><?= count(array_filter($didoxDocuments, function ($d) {
                                            return $d->didox_id; })) ?></strong>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="alert alert-info text-center">
                        ...
                        <div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-radius: 5px;">
                            ...
                            <div style="margin-top: 10px;">
                                <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/create-didox-invoice', 'id' => $model->id]); ?>"
                                    class="btn btn-info" onclick="return confirm('Создать счет-фактуру автоматически?')">
                                    <i class="fa fa-bolt"></i> Авто: Счет-фактура
                                </a>
                                <a href="<?= Yii::$app->urlManager->createUrl(['/admin/order/create-didox-arbitrary', 'id' => $model->id]); ?>"
                                    class="btn btn-info"
                                ...
                                <?php foreach ($didoxLogs as $log): ?>
                                    ...
                                    <tr class="<?= $rowClass ?>">
                                        ...
                                        <td>
                                            ...
                                            <?php if ($log->data): ?>
                                                <br><small class="text-muted"
                                                    style="font-family: monospace;"><?= \yii\helpers\StringHelper::truncate(Html::encode($log->data), 200) ?></small>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
