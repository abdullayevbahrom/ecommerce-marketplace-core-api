            ...
            <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data'], 'id' => 'form-profile']);?>
                <div class="box-body">
                    ...
                    <div class="lang-block lang-block-ru">
                        <?=$form->field($model, 'name_ru', ['template'=>'{label}<div class="input-group"><span class="input-group-addon"><i class="fa fa-pencil"></i></span>{input}</div>{error}'])->textInput()->input('text')->label('Name (RU)');?>
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <?=$form->field($model, 'link', ['template'=>'{label}<div class="input-group"><span class="input-group-addon"><i class="fa fa-pencil"></i></span>{input}</div>{error}'])->textInput()->input('text')->label('Link');?>
                        </div>
                        <div class="col-sm-6">
                            <?=$form->field($model, 'type')->dropDownList(
                                ['site' => 'Site', 'mobile' => 'Mobile'],
                                [
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <?php if ($model->image) {?>
                                ...
                                <br/><br/>
                                <a href="<?=Yii::$app->urlManager->createUrl(['/admin/default/remove-photo', 'id'=>$model->image->id])?>" class="remove-object btn btn-sm btn-danger"><i class="fa fa-remove"></i> Delete photo</a>
                                <br/><br/>
                            <?php }?>
                        </div>
                    </div>    
