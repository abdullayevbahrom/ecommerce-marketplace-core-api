...
$this->title = 'Edit Product Type: ' . $model->name_ru;
...
<div class="content-wrapper">
    ...
    <section class="content">
        ...
        <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]);?>
            ...
            <div class="box box-info color-palette-box" id="values-section" style="display: <?= ($model->type == 'select' || $model->type == 'checkbox') ? 'block' : 'none' ?>">
                <div class="box-header">
                ...
                <div class="box-body">
                    <div class="alert alert-info">
                        <i class="fa fa-info-circle"></i> These values will be available for selection when this type is used in products. You can edit existing values or add new ones.
                    </div>
                    ...
                    <div id="values-container">
                        <?php if ($model->productTypeValues) { ?>
                            <?php foreach ($model->productTypeValues as $index => $value) { ?>
                                <div class="value-item" style="margin-bottom: 15px; padding: 10px; border: 1px solid #e3e3e3; border-radius: 4px; background-color: #fafafa;">
                                    <input type="hidden" name="values[<?=$index?>][id]" value="<?=$value->id?>">
                                    <div class="row">
                                        <div class="col-sm-3">
                                            <input type="text" name="values[<?=$index?>][value_ru]" class="form-control" placeholder="Value (Russian)" value="<?=htmlspecialchars($value->value_ru)?>" required>
                                        </div>
                                        <div class="col-sm-3">
                                            <input type="text" name="values[<?=$index?>][value_en]" class="form-control" placeholder="Value (English)" value="<?=htmlspecialchars($value->value_en)?>">
                                        </div>
                                        <div class="col-sm-3">
                                            <input type="text" name="values[<?=$index?>][value_uz]" class="form-control" placeholder="Value (Uzbek)" value="<?=htmlspecialchars($value->value_uz)?>">
                                        </div>
                                        <div class="col-sm-2">
                                            <input type="number" name="values[<?=$index?>][sort]" class="form-control" placeholder="Sort" value="<?=$value->sort?>" min="0">
                                        </div>
                                        <div class="col-sm-1">
                    ...
                    <div class="text-center" id="no-values-message" style="padding: 20px; color: #999; <?= ($model->productTypeValues && count($model->productTypeValues) > 0) ? 'display: none;' : '' ?>">
...
function showValuesSection() {
    ...
    // Auto-add first value if no visible values exist
    if ($('#values-container .value-item:visible').length === 0) {
        setTimeout(function() {
            addNewValue();
        }, 100);
    }
...
function addNewValue() {
    ...
    var html = '<div class="value-item" style="margin-bottom: 15px; padding: 10px; border: 1px solid #e3e3e3; border-radius: 4px; background-color: #fafafa;">' +
               '<input type="hidden" name="values[' + index + '][id]" value="">' + // New values have no ID initially
               '<div class="row">' +
               '<div class="col-sm-3">' +
...
// Simple function to remove value (handles both existing and new values)
function removeValue(button) {
    console.log('removeValue called'); // Debug
    
    if (confirm('Are you sure you want to remove this value?')) {
        var valueItem = $(button).closest('.value-item');
        var hiddenId = valueItem.find('input[type="hidden"][name*="[id]"]');
        
        // If this is an existing value (has an ID), mark it for deletion
        if (hiddenId.length > 0 && hiddenId.val()) {
            console.log('Marking existing value for deletion:', hiddenId.val());
            // Add a hidden field to mark this value for deletion
            valueItem.hide();
            valueItem.find('input').attr('disabled', 'disabled');
            valueItem.append('<input type="hidden" name="deleted_values[]" value="' + hiddenId.val() + '">');
        } else {
            console.log('Removing new value');
            // This is a new value, just remove it
            valueItem.remove();
        }
...
function updateNoValuesMessage() {
    var visibleValueCount = $('#values-container .value-item:visible').length;
    if (visibleValueCount > 0) {
...
// Function to reindex values after removal (only for visible items)
function reindexValues() {
    var index = 0;
    $('#values-container .value-item:visible').each(function() {
        ...
        $(this).find('input[type="hidden"][name*="[id]"]').attr('name', 'values[' + index + '][id]');
        index++;
    });
}

$(document).ready(function() {
    // Function to handle type change (works with both regular select and Select2)
    function handleTypeChange() {
        var selectedType = $('#producttype-type').val() || $('select[name="ProductType[type]"]').val();
    ...
    // Initialize on page load - check initial value and set button state
    setTimeout(function() {
        handleTypeChange();
        
        // Set initial button state based on section visibility
        if ($('#values-section').is(':visible')) {
            $('#show-values-btn').text('Values Section Shown').removeClass('btn-info').addClass('btn-success');
        } else {
            $('#show-values-btn').text('Add More Values').removeClass('btn-success').addClass('btn-info');
        }
    ...
    $('form').on('submit', function(e) {
        ...
        if (selectedType === 'select' || selectedType === 'checkbox') {
            ...
            var emptyValues = [];
            $('#values-container .value-item:visible').each(function(index) {
                var ruValue = $(this).find('input[name*="[value_ru]"]').val().trim();
                if (ruValue === '') {
