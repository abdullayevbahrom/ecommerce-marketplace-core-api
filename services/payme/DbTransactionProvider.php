<?php
namespace yii\services\payme;

require 'TransactionProvider.php';
...
class DbTransactionProvider implements TransactionProvider
{
     ...
     * @return mixed
     */
    public function getByOwnerId($ownerId, $orderId)
    {
        $trans = $this->db->query("SELECT * FROM {$this->tableName} WHERE user_id = '{$ownerId}' AND order_id = '{$orderId}'")
            ->fetch();
    ...
    {
        $sql = "INSERT INTO `{$this->tableName}` (transaction, payme_time, amount, state, create_time, user_id, order_id) 
            VALUES ('{$fields['transaction']}', '{$fields['payme_time']}', {$fields['amount']}, {$fields['state']}, {$fields['create_time']}, {$fields['user_id']}, {$fields['id']})";
